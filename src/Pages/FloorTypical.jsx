...
import TypicalMap from "../components/TypicalMap";
import Tippy from "@tippyjs/react";
import "tippy.js/dist/tippy.css";
import { useParams } from "react-router-dom";

export default function FloorTypical() {
  const images = [T1.typical];
  ...
  return (
    <div className="fixed inset-0  overflow-hidden bg-[#9A8570]">
      ...
      >
            ...
            >
              ...
              <div className="flex items-center p-0 ">
                <div className="bg-white flex justify-center items-center rounded-lg px-2 py-0 w-24 h-7">
                  <h1 className="text-sm">Floor{params.Id}</h1>
                </div>
              </div>
            </div>
        ...
        {/* Scroll bar menu */}
        <div
          className={`fixed left-40 top-29 z-50  ${
            opacity ? "opacity-15" : ""
          } rounded-2xl text-white flex flex-col items-center justify-between  scale-70 origin-top-right p-1 transition-all duration-500 ease-in-out`}
        >
          <div
            className={`bg-[#333333]/90 text-white p-4 rounded-lg w-65 ${
              !close ? "h-125" : "h-30"
          ...
          >
            ...
            <div className="mb-4">
              ...
              {!close && (
                <div
                  className={`overflow-hidden mb-4 transition-all duration-500 ease-in-out ${
                    close ? "h-0 opacity-0" : "h-10 opacity-100"
                  }`}
                >
                  <input
                    type="text"
            ...
            {!close && (
              <div
                className={` overflow-hidden mb-4 transition-all duration-500 ease-in-out ${
                  close ? "h-0 opacity-0" : "h-25 opacity-100"
                }`}
              >
            ...
            {!close && (
              <div
                className={` overflow-hidden transition-all duration-500 ease-in-out ${
                  close ? "h-0 opacity-0" : "h-50 opacity-100"
                }`}
              >
            ...
            <button
              ...
              } text-black cursor-pointer rounded text-sm  transition-all duration-500`}
            >
