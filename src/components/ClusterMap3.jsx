...
import { t5_t6_frames_svg } from "../assets/data/svgs/towers";
...
function ClusterMap3() {
  ...
  const addInteractivity = (
    ...
    if (element.type === "g" && !parentIds.tower) {
      parentIds.tower = `T${element.props.id}`; // Convert to match API format
      ...
      children: React.Children.map(element.props.children, (child) => {
        ...
        if (child.type === "g") {
          ...
          return (
            ...
            >
              <Link
                to={`/cluster3/${parentIds.tower.split("_")[0]}/floor/${
                  parentIds.floor.split("_")[0]
                }`}
              >
                {clonedPath}
  ...
  return (
    <div className="absolute inset-0 w-full h-full">
      ...
      >
        {addInteractivity(t5_t6_frames_svg[clusterCount])}
      </svg>
    </div>
  );
}
