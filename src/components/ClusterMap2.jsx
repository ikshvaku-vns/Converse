...
import { t3_t4_frames_svg } from "../assets/data/svgs/towers";
...
function ClusterMap2() {
  ...
  const addInteractivity = (
    ...
    if (element.type === "g" && !parentIds.tower) {
      const firstDigit = element.props.id.match(/\d+/)[0]; // Extract first number
      parentIds.tower = `T${+firstDigit + 2}`; // Convert to match API format
      console.log("Tower ID:", parentIds.tower);
    }
      ...
      children: React.Children.map(element.props.children, (child) => {
        ...
        if (child.type === "g") {
          ...
          return (
            ...
            >
              <Link
                to={`/cluster2/${parentIds.tower.split("_")[0]}/floor/${
                  parentIds.floor.split("_")[0]
                }`}
              >
                {clonedPath}
  ...
  return (
    <div className="absolute inset-0 w-full h-full">
      ...
      >
        {addInteractivity(t3_t4_frames_svg[clusterCount])}
      </svg>
    </div>
  );
}

export default ClusterMap2;
